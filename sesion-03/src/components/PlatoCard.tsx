  ...
  return (
    <div className="bg-white rounded-2xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden border border-gray-100 group">
      <div className="relative h-48 overflow-hidden">
        <img className="w-gull h-full object-cover group-hover:scale-110 transition-transform duration-500" src={`${API_CONFIG.BASE_URL}${plato.imagen}`} alt={plato.nombre} />
        ...
        <div className="p-6">
          <div className="flex justify-between mb-3">
            ...
            <span className="text-sm font-medium text-gray-500 bg-gray-50 px-2 py-2 rounded">{plato.calorias}</span>
          </div>
          <div>
            ...
            <div>
              {plato.ingredientes.map((ingrediente:string,index:number) => (
                <span key={index} className="text-[10px] px-2 py-1 bg-orange-50 text-orange-600 roundend-md border-orange-100">
                  {ingrediente}
                </span>
              ))}
            </div>
